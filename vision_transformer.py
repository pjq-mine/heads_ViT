...
class Attention(nn.Module):
    ...
    def forward(self, x, prev_attn_logits):
        B, N, C = x.shape
        ...
        if prev_attn_logits is not None:
            ...
            prev_attn_logits = prev_attn_logits + attn
            fused_prev_logits = self.head_ffn(prev_attn_logits.transpose(1, 3)).transpose(3, 1)
            #attn = 0.7*attn + fused_prev_logits + 0.3*residual_logits
            attn = attn + fused_prev_logits+ residual_logits
        
        if self.training:
            noise = torch.randn_like(attn)
            attn = attn + noise
        attn0 = attn

        attn = attn0.softmax(dim=-1)
...
class Block(nn.Module):
    ...
    def forward(self, x, prev_attn_logits):
        residual = x
        out = self.norm1(x)
        out, self_attn_logit = self.attn(out, prev_attn_logits)
        out = self.ls1(out)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            print('-------checkpoint_seq-------')
            x = checkpoint_seq(self.blocks, x)
        else:
            # x = self.blocks(x)
            self_attn_logit = None
            for layer in self.blocks:
                x, self_attn_logit = layer(x, prev_attn_logits=self_attn_logit)
        x = self.norm(x)
        return x, self_attn_logit
    ...
    def forward(self, x):
        x, self_attn_logit = self.forward_features(x)
